...
WITH first_item(customer_id, product_name, order_date, rnk) 
as(
...
-- 5. Which item was the most popular for each customer?
WITH rnk(customer_id, product_name, product_id, cnt, rnk) 
AS (
    SELECT s.customer_id
	,  m.product_name
	,  s.product_id
...
WITH members_orders(customer_id, order_date, product_name, rnk)
AS(
    SELECT s.customer_id
	,  s.order_date
	,  m.product_name
